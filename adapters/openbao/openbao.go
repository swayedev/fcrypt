// Package openbao provides an OpenBao transit adapter for fcrypt key wrapping.
package openbao
...
// Config configures an OpenBao transit client.
type Config struct {
	Address    string
	Token      string
	Mount      string
...
// Client wraps and unwraps data keys using OpenBao transit.
type Client struct {
	address string
	token   string
	mount   string
...
// New creates an OpenBao transit adapter.
...
// WrapKey wraps plaintextKey with OpenBao transit.
func (c *Client) WrapKey(ctx context.Context, plaintextKey []byte, opts fcrypt.WrapOptions) (fcrypt.WrappedKey, error) {
	...
	return fcrypt.WrappedKey{
		KeyID:      keyName,
		Algorithm:  "openbao-transit",
		Ciphertext: []byte(ciphertext),
		AAD:        cloneBytes(opts.AAD),
	}, nil
}

// UnwrapKey unwraps a key with OpenBao transit.
...
func (c *Client) call(ctx context.Context, action string, keyName string, payload map[string]string) (map[string]string, error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		return nil, fmt.Errorf("%w: openbao transit status %d", fcrypt.ErrInvalidWrappedKey, httpResp.StatusCode)
	}
